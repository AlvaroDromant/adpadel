<x-app-layout>
    ...
    <div>
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <!-- Search Form -->
            <form method="GET" action="{{ route('user.userpalas') }}" class="mb-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <div>
                        <label for="marca" class="block text-sm font-medium text-gray-700">Marca</label>
                        <input type="text" name="marca" id="marca" value="{{ request('marca') }}" class="mt-1 block w-full">
                    </div>

                    <div>
                        <label for="nivel_juego" class="block text-sm font-medium text-gray-700">Nivel de Juego</label>
                        <select name="nivel_juego" id="nivel_juego" class="mt-1 block w-full">
                            <option value="">Todos</option>
                            <option value="principiante" {{ request('nivel_juego') == 'principiante' ? 'selected' : '' }}>Principiante</option>
                            <option value="intermedio" {{ request('nivel_juego') == 'intermedio' ? 'selected' : '' }}>Intermedio</option>
                            <option value="avanzado" {{ request('nivel_juego') == 'avanzado' ? 'selected' : '' }}>Avanzado</option>
                        </select>
                    </div>

                    <div>
                        <label for="precio_min" class="block text-sm font-medium text-gray-700">Precio Mínimo</label>
                        <input type="number" name="precio_min" id="precio_min" value="{{ request('precio_min') }}" class="mt-1 block w-full">
                    </div>

                    <div>
                        <label for="precio_max" class="block text-sm font-medium text-gray-700">Precio Máximo</label>
                        <input type="number" name="precio_max" id="precio_max" value="{{ request('precio_max') }}" class="mt-1 block w-full">
                    </div>
                </div>

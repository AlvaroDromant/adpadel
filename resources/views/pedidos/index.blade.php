    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        @foreach ($pedidos as $pedido)
            <div class="bg-white shadow-lg rounded-lg mb-6 overflow-hidden">
                <div class="px-6 py-5 bg-gradient-to-r from-green-400 to-blue-500 text-white">
                    <h3 class="text-xl leading-6 font-semibold">
                        Pedido #{{ $pedido->id }}
                    </h3>
                    <p class="mt-1 max-w-2xl text-sm">
                        Realizado el {{ $pedido->created_at->format('d/m/Y') }}
                    </p>
                </div>
                <div class="border-t border-gray-200">
                    <dl>
                        @foreach ($pedido->items as $item)
                            <div class="bg-gray-50 px-6 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-700">
                                    {{ $item->pala->marca }} - {{ $item->pala->modelo }}
                                </dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <span class="block">Cantidad: {{ $item->cantidad }}</span>
                                    <span class="block">Precio: {{ $item->precio }} €</span>
                                </dd>
                            </div>
                        @endforeach
                        <div class="bg-white px-6 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <span class="inline-block px-3 py-1 font-semibold text-sm leading-tight rounded-full 
